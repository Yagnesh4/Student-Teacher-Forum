...
app.get('/answerPage',checkStudent,ifNotAuthenticated,  function(req,res){
   QuestionChar.find({}, function(err,data){
	  if(req.isAuthenticated()){
		...
		if(j>=data.length){
			j=0 
			// res.render('answerPage', {name: req.user.name, msg:"NO QUEST|I", data: data[j]})
			res.send("NO MORE QUESTIONS")
		}  
		res.render('answerPage', {name: req.user.name,msg:"NO NEW QUESTIONS TO ANSWER", data: data[j]})
		i=j++
		console.log(j)
